...
const paths = [[new Vec(0, 9), new Vec(2, 5), new Vec(4, 9)],
...
function drawWinBox() {
	...
	drawText(starStr(calcStars()), BOX_CENTER_X, BOX_Y + EDGE_RADIUS * 1.5, STAR_FONT, {centerX: true, color: "gold"});
	nextButton.draw();
	retryButton.draw();
}
function drawGame() {
	...
	ctx.font = UI_FONT;
	drawText(levelStr, EDGE_RADIUS * 23, EDGE_RADIUS, UI_FONT, {spacing: 1.25});
	drawText(starStr(stars[level]), EDGE_RADIUS * 22.5, EDGE_RADIUS * 1.2, STAR_FONT, {color: "gold"});
	drawSchema();
	drawText(
	`Moves: ${player.moves}
Target: ${target}
	`,
	EDGE_RADIUS * 23, EDGE_RADIUS * 3.5, UI_FONT, {spacing: 1.25});
	...
	if (player.moves > target + 9 && level < paths.length) {
		const availableHints = Math.min(Math.floor((player.moves - (target + 10)) / 5) + 1, paths[level].length - 1);

    if (numHint < availableHints && numHint <= paths[level].length - 1) {
        hintButton.draw();
        ctx.beginPath();
        ctx.arc(EDGE_RADIUS * 28.6, BUTTON_Y - EDGE_RADIUS * 0.8, 0.3 * EDGE_RADIUS, 0, Math.PI * 2);
        ctx.fillStyle = "lightpink";
        ctx.fill();
        drawText(`${availableHints - numHint}`, EDGE_RADIUS * 28.5, BUTTON_Y - EDGE_RADIUS, EDGE_RADIUS * 0.5 + "px monospace")
    }
		if (hint) showHint();
	}
}
...
function drawSchema() {
	...
	if (!level) {
		...
		if (level == 3) {
			...
			drawText(`You stops at`, x - EDGE_RADIUS, EDGE_RADIUS * 9.5, UI_FONT);
			drawHex(x, EDGE_RADIUS * 11.4, "SAND", "#ff7");
		} else if ( level == 14 ) {
			drawRec(x, EDGE_RADIUS * 8, EDGE_RADIUS * 4, EDGE_RADIUS * 9 - 5);
			drawText(`You bounces off`, x - EDGE_RADIUS, EDGE_RADIUS * 9.5, UI_FONT);
			drawText("MIRROR", EDGE_RADIUS * 26, EDGE_RADIUS * 11.4 - 0.3 * EDGE_RADIUS, UI_FONT);
			...
			drawText(`Change direction by`, x - EDGE_RADIUS, EDGE_RADIUS * 9.5, UI_FONT);
			drawText("REDIRECTOR", EDGE_RADIUS * 26, EDGE_RADIUS * 11.4 - 0.3 * EDGE_RADIUS, UI_FONT);
			...
			drawHex(x, EDGE_RADIUS * 11.4, "Switch", "violet", 0.5);
		}
	}
}
...
canvas.addEventListener("pointerdown", e => {
	...
	if (inMenu) {
		...
		hint = hintButton.hex.contains(mouse);
		...
		if (reset) resetLevel();
		if (menu) enterMenu();
		if (player.moves > 2 && level < paths.length) {
			if (hint) numHint ++;
		}
		if (player.winning) {
